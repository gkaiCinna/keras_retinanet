...
from ..utils.anchors import anchors_for_shape, compute_gt_annotations, compute_overlap
...
def parse_args(args):
    ...
    parser.add_argument('--save-dir', help='Path to a save debugged images.')
...
def run(generator, args, anchor_params):
    ...
    """
    save_path = os.path.join(os.getcwd(), args.save_dir)
    if os.path.exists(save_path):
        print('Delete existing folder: {}'.format(save_path))
        shutil.rmtree(save_path, ignore_errors=False, onerror=None)

    if not os.path.exists(save_path):
        print('Created folder: {}'.format(save_path))
        os.mkdir(save_path)

    # iou thresholds list
    iou_thresholds = np.linspace(0.50, 0.95, np.round((0.95 - 0.50) / 0.05) + 1)
    iou_overlaps = {iou : [0, 0] for iou in iou_thresholds}
    ...
    for i in progressbar.progressbar(range(generator.size()), prefix='Running network: '):
        ...
        if len(annotations['labels']) > 0 :
            ...
            # label_bboxes = annotations['bboxes'][annotations['labels']==label]
            # resize the image and annotations
            if args.resize:
                image, image_scale = generator.resize_image(image)
                annotations['bboxes'] *= image_scale

            anchors = anchors_for_shape(image.shape, pyramid_levels=[2, 3, 4, 5, 6, 7], anchor_params=anchor_params)
            positive_indices, _, max_indices = compute_gt_annotations(anchors, annotations['bboxes'])

            # calculate overlaps per anchor
            for anno in annotations['bboxes']:
                overlaps            = compute_overlap(anchors[positive_indices], np.expand_dims(anno, axis=0))
                assigned_anchors = np.argmax(overlaps, axis=0)
                max_overlap         = overlaps[assigned_anchors, 0]

                for iou_th in iou_thresholds:
                    iou_overlaps[iou_th][1] += 1
                    if max_overlap >= iou_th:
                        iou_overlaps[iou_th][0] += 1
            ...
            save_img_path = os.path.join(save_path, os.path.basename(generator.image_path(i)))
            cv2.imwrite(save_img_path, image)

    print()
    print('Total amount:\t{}'.format(total_amount))
    for iou in iou_thresholds:
        print('IOU threshold:\t{}'.format(iou))
        print('Hit:           \t{}'.format(round(iou_overlaps[iou_th][0], 2)))
        print('Overlap ratio:\t{}%'.format(100*round(iou_overlaps[iou_th][0]/iou_overlaps[iou_th][1], 3)))
